...
#This is your trial string (short version)
employee_string="Rudy T. Navarro,586 New St.,Anchorage,Jennie K. Carlson,468 West Old Drive,Charlotte,Kristine Z. Blackwell,363 Yellow Duck Way,Fresno,Ivan N. Marquez,876 Rocky Second Street,Yonkers,Clarence H. Baird,915 North Nobel St.,San Antonio,Tia J. Li,780 Rocky Cherrywood Street,Indianapolis,Tammy O. Holder,882 West Crabapple Avenue,Toledo,Nicole K. Perkins,58 Old St.,El Paso,Spencer M. Cross,48 North Narwhal St.,Atlanta,Robin T. Fernandez,51 West Rocky Lakeridge St.,Grand Rapids,Cindy J. Oconnor,925 Green New Parkway,Atlanta"

#check for the subdirectory mod08t
dircheck=~/mod08t
if ! [[ -d $dircheck ]];then
	mkdir ~/mod08t
fi
#clean up employee string for array, replace delimitors
employee_string=$(echo $employee_string | tr " " "_")
employee_string=$(echo $employee_string | tr "," " ")

employee_array=($(echo $employee_string | cut -d " " -f 1-))

clear
echo "Making employee file.."

#make .csv file of employee data
echo "Name,Address,City" > ~/mod08temployee.csv #destructive write on purpose

#write the employee data out to employee.csv in the mod08t directory
array_len=${#employee_array[*]}
for (( i=0; i<$array_len; i=$(($i +3)) )) #increment by 3 because each csv entry has 3 elements
do
	line_out="${employee_array[$i]},\
${employee_array[$(($i+1))]},\
${employee_array[$(($i+2))]}"
	echo $line_out >> ~/mod08t/employee.csv
done


#make city array from employee.csv
city=()
#do not put quotes around the path!
input=~/mod08t/employee.csv
while IFS= read -r line
do
	#first line here skips the first line of .csv header
...
echo -e "\nMaking array of cities\n"

#sort and unique the array of cities
...
#add last names to city string in array
echo "Adding last names, this will take a while..."
while IFS= read -r line
do
	full_name=$(echo $line | cut -d "," -f 1)
	last_name=$(echo $full_name | cut -d "_" -f 3)
	city=$(echo $line | cut -d "," -f3)
	for i in ${!city_array[*]};
	do
		city_from_array=$(echo ${city_array[$i]} | cut -d "|" -f 1)
		if [[ $city_from_array == $city ]];then
			city_array[$i]+="|$last_name"
		fi
	done
done < $input

#print out the cities and last names in a clean fashion
clear
for i in ${city_array[*]}
do
	temp_array=($(echo $i | tr "|" " " | cut -d " " -f 1-))
	echo "${temp_array[0]}: ${temp_array[*]:1}" | tr "_" " "
done

#pack up employee_data.csv into the mod08t directory
cd ~/mod08t
